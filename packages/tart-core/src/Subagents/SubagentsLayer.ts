...
import type { RealizedTartTool, TartTool } from '../Api/ToolDefinition'
...
export type SubagentsConfig = {
	...
	readonly realizeAgentTools: (tools: ReadonlyArray<TartTool>) => RealizedAgentTools
...
export const makeSubagents = (
	...
	Effect.gen(function* () {
		...
		const continueSubagent: SubagentsService['continueSubagent'] = Effect.fn('tart.subagents.continue')(
		...
		const dispatch: SubagentsService['dispatch'] = Effect.fn('tart.subagents.dispatch')(
		...
		const fork: SubagentsService['fork'] = Effect.fn('tart.subagents.fork')((input: ForkSubagentInput) =>
			Effect.gen(function* () {
				const dispatcher = yield* CurrentAgent
				const currentCall = yield* CurrentToolCall
				const interruptNote = yield* InterruptNote
		...
		const resume: SubagentsService['resume'] = Effect.fn('tart.subagents.resume')((input: ResumeSubagentInput) =>
			Effect.gen(function* () {
				const dispatcher = yield* CurrentAgent
				const currentCall = yield* CurrentToolCall
				const interruptNote = yield* InterruptNote
