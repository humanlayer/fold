 ...
 * carries the merged `TartEvent` shape that Session exposes: durable `log` rows (folded in by
...
export type AgentEventsService = {
	...
	readonly publish: (event: TartEvent) => Effect.Effect<void>

	/** A live-only stream of published events; each run establishes a fresh subscription with no replay. */
	readonly subscribe: Stream.Stream<TartEvent>
}

/** Ephemeral event spine. One provided instance represents one session, mirroring EventLog. */
export class AgentEvents extends Context.Service<AgentEvents, AgentEventsService>()('tart/AgentEvents') {}
