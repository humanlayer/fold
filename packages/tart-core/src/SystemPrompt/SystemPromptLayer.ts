 ...
 * agent's own blocks (D17). tart-core ships no baked base prompts yet, so the default layer composes
...
export const makeSystemPrompt = (options?: MakeSystemPromptOptions): Layer.Layer<SystemPrompt> =>
	Layer.succeed(SystemPrompt, {
		compose: Effect.fn('tart.system_prompt.compose')((input: ComposeSystemPromptInput) =>
			Effect.sync(() => {
