 ...
 * resolve through at each dispatch/resume, so `TartSession.setProfile` swaps a provider once and every
...
/** Narrow a `TartModel | ProfileRole` binding: a role is a literal string, a model is an object. */
 ...
 * status as `TartModel` itself); nothing durable is written from this map - role bindings resolve to
...
export const profileModelFor = (profiles: SessionProfiles, role: ProfileRole): TartModel | undefined =>
...
export type ProfilesService = {
	 ...
	 */
	readonly resolve: (role: ProfileRole) => Effect.Effect<TartModel>
	/** Rebind one role; every subsequent dispatch/resume of a role-bound type sees the new model. */
	readonly set: (role: ProfileRole, model: TartModel) => Effect.Effect<void>
...
export class Profiles extends Context.Service<Profiles, ProfilesService>()('tart/Profiles') {}
...
export const makeProfiles = (initial: SessionProfiles): Effect.Effect<ProfilesService> =>
	Effect.gen(function* () {
		...
		const resolve = (role: ProfileRole): Effect.Effect<TartModel> =>
			Ref.get(state).pipe(
		...
		const set = (role: ProfileRole, model: TartModel): Effect.Effect<void> =>
