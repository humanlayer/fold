 ...
 * fails with `SessionAlreadyStartedError`. `send` runs one user turn on the root agent and resolves with
 * the durable `agent-finished` entry; a call before `start` fails with `SessionNotStartedError`.
 ...
 * `events` merges durable log rows with ephemeral deltas into one error-free stream. `interrupt` is a
 * Slice A stub.
 */
