...
export type OutputRenderer = {
	...
	readonly renderEvent: (event: FoldEvent) => Effect.Effect<void>
	...
	return {
		renderHeader: (header) =>
			Effect.sync(() => {
				...
				Effect.andThen(
					writeStdout(
						`${ansi.bold('fold')} ${header.mode === 'new' ? ansi.green('new session') : ansi.cyan('resumed session')} ${header.sessionId}\n` +
		...
		prompt: Effect.succeed(`${ansi.green('fold')} ${ansi.dim('>')} `),
	}
}
