...
const delta = (type: 'text-delta' | 'reasoning-delta', id: string, text: string, agentId = rootAgentId): FoldEvent => ({
	kind: 'delta',
	agentId,
	parentAgentId: agentId === rootAgentId ? null : rootAgentId,
	toolCallId: null,
...
describe('TUI session reducer', () => {
	...
	it('does not let child lifecycle entries replace the root model or status', () => {
		...
		expect(finishedChild.reasoningLevel).toBe('off')
		expect(finishedChild.status).toBe('RUNNING')
	})

	it('projects root model and reasoning changes for the header', () => {
		const started = entry({
			_tag: 'agent_started',
			seq: 0,
			ts: 0,
		...
		const changed = entry({
			...
			toolCallId: null,
			reasoningLevel: 'high',
			reason: 'configured in the TUI',
		})

		const state = reduceSessionEvents(
			makeSessionState(1),
			[started, changed].map((value) => ({ kind: 'log' as const, entry: value })),
			rootAgentId,
		)

		expect(state.model).toBe('root-model')
	...
	it('folds tool results into their call row without exposing result contents', () => {
		...
		expect(rows.some(({ text }) => text.includes('fold-cli'))).toBe(false)
	})
	...
	it('is deterministic for the same initial state and events', () => {
		...
		] satisfies ReadonlyArray<FoldEvent>
