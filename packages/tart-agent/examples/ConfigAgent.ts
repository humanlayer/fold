 ...
 * Loads `~/.tart/config.jsonc` (writing a commented starter on first run), resolves the mode's `smart`
 * role to a model, folds this directory's agentfiles (AGENTS.md/CLAUDE.md) into the system prompt, and
 * runs the default coding mode over a JSONL session log under `~/.tart/sessions`.
 *
 * First run:  bun packages/tart-agent/examples/ConfigAgent.ts
 *   -> writes ~/.tart/config.jsonc; edit providers/roles, export the referenced API key env var, re-run.
 * Then:       bun packages/tart-agent/examples/ConfigAgent.ts "your prompt"
 */
import { Console, Effect } from 'effect'

import { configInit, launchSession, loadTartConfigOrNull } from '../src/index'

const prompt = process.argv[2] ?? 'List the files in the current directory and briefly summarize this project.'

const program = Effect.gen(function* () {
	const config = yield* loadTartConfigOrNull()
	if (config === null) {
