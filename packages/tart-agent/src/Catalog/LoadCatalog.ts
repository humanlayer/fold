 ...
 * (`<tartHome>/cache/models-dev.json`, TTL 24h) short-circuits everything; otherwise models.dev is
 ...
 * catalog. `TART_DISABLE_MODELS_FETCH` (env seam) skips the fetch entirely. The loader NEVER fails -
 ...
 *
 * Seams follow the tart-agent options convention (AgentFiles/Load.ts): `fileSystem` for hermetic
 * tests, `env` for the disable flag, `fetchJson` for the network, `now`/`ttlMs` for freshness.
 */
import { dirname, join } from 'node:path'

import { ModelCatalogEntry } from '@humanlayer/tart-core'
...
export const TART_DISABLE_MODELS_FETCH = 'TART_DISABLE_MODELS_FETCH'
...
export type LoadModelCatalogOptions = {
	/** The tart home directory; the cache lives at `<tartHome>/cache/models-dev.json`. */
	...
	/** Environment lookup for {@link TART_DISABLE_MODELS_FETCH}. Defaults to reading `process.env`. */
...
/** The cache file path for a tart home directory. */
export const modelCatalogCachePath = (tartHome: string): string => join(tartHome, 'cache', 'models-dev.json')
...
export const loadModelCatalog = (options: LoadModelCatalogOptions): Effect.Effect<ReadonlyArray<ModelCatalogEntry>> =>
	Effect.gen(function* () {
		...
		const cachePath = modelCatalogCachePath(options.tartHome)
		...
		const disableFlag = env(TART_DISABLE_MODELS_FETCH)
