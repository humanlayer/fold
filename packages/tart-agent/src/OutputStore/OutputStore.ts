 ...
 * `<tartHome>/tool-output/<sessionId>/<toolCallId>.txt`. The durable log keeps the truncated result
...
import { defaultTartHome } from '../Config/Load'
import { fileSystemFor, type FsToolOptions } from '../Fs/DefaultFileSystem'

const dayMs = 24 * 60 * 60 * 1000

/** Reference to one stored tool-output file. */
export class OutputStoreRef extends Schema.Class<OutputStoreRef>('tart-agent/OutputStoreRef')({
	sessionId: SessionId,
...
export class OutputStore extends Context.Service<OutputStore, OutputStoreService>()('tart-agent/OutputStore') {}
...
export const toolOutputRootFor = (options?: { readonly tartHome?: string }): string =>
	join(options?.tartHome ?? defaultTartHome(), 'tool-output')

/** Directory for one session's stored tool output. */
export const toolOutputSessionDirFor = (input: { readonly sessionId: SessionId; readonly tartHome?: string }): string =>
...
export const makeOutputStore = (options: MakeOutputStoreOptions): OutputStoreService => {
	...
	const tartHome = options.tartHome ?? defaultTartHome()
	const sessionId = options.sessionId
	const directory = toolOutputSessionDirFor({ sessionId, tartHome })
	...
	const refFor = (toolCallId: ToolCallId): OutputStoreRef =>
		new OutputStoreRef({
			sessionId,
			toolCallId,
			path: toolOutputPathFor({ sessionId, toolCallId, tartHome }),
		})
	...
	const sweep = Effect.gen(function* () {
		const root = toolOutputRootFor({ tartHome })
