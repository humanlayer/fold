 ...
 * plus a fetch failure degrades to the baked snapshot; TART_DISABLE_MODELS_FETCH skips the fetch
...
import {
	...
	TART_DISABLE_MODELS_FETCH,
} from '../../src/index'
import { memoryFileSystem } from '../TestHelpers'

const tartHome = '/home/user/.tart'
const cachePath = modelCatalogCachePath(tartHome)
...
it.effect('TART_DISABLE_MODELS_FETCH skips the fetch: stale cache when present, baked otherwise', () =>
	Effect.gen(function* () {
		const env = (name: string): string | undefined => (name === TART_DISABLE_MODELS_FETCH ? '1' : undefined)
