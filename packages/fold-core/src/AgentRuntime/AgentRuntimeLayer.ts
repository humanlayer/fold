 ...
 * LanguageModel, persist the assistant message with fold tool-call ids (stashing provider ids in part
...
> = Layer.effect(
	...
	Effect.gen(function* () {
		...
		/** Rewrite provider tool-call ids to freshly minted fold ids, stashing the provider id per part. */
		...
		const start: AgentRuntimeService['start'] = Effect.fn('fold.agent_runtime.start')((input: StartAgentInput) =>
			Effect.gen(function* () {
		...
		const switchModel: AgentRuntimeService['switchModel'] = Effect.fn('fold.agent_runtime.switch_model')(
		...
		const run: AgentRuntimeService['run'] = Effect.fn('fold.agent_runtime.run')((input: RunAgentInput) =>
			Effect.gen(function* () {
		...
		const compact: AgentRuntimeService['compact'] = Effect.fn('fold.agent_runtime.compact')(
