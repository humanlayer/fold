...
describe('provider auth configuration', () => {
	it('maps the configured provider name and tart home to the shared auth store', () => {
		expect(codexAuthStoreOptions('work-codex', '/tmp/tart-home')).toEqual({
			providerId: 'work-codex',
			path: '/tmp/tart-home/auth.json',
		})
	})
