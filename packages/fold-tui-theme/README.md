...
aesthetic to carry into the real Fold TUI. Every theme renders exactly the same data; press
...
feed — deliberately **not** Effect, even though the rest of Fold is. The point here is the
pixels, and Effect would only add ceremony between you and the frame. The real Fold TUI will
