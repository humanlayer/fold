 ...
 * `openai` providers, plus the static xAI frontier entries from `fold-xai`, all normalized to core
 * `ModelCatalogEntry` rows. Reads a local snapshot from `/tmp/models_dev_api.json` when present
 * (avoid hammering models.dev while iterating), otherwise fetches `https://models.dev/api.json` fresh.
 *
