import { shortAgentId, type AgentId, type CompactionArchiveAccessService } from '@humanlayer/fold-core'
/**
 * fold-agent's host implementation of post-compaction archive access instructions. Core persists and
 * renders the returned string, but only fold-agent knows the JSONL session path and whether the root is
...
/** Options for the fold-agent archive-access service. */
...
/** Build fold-agent's live CompactionArchiveAccess service for one JSONL-backed session. */
export const compactionArchiveAccessFor = (
	options: CompactionArchiveAccessOptions,
): CompactionArchiveAccessService => ({
	instructions: Effect.fn('fold_agent.compaction_archive_access.instructions')((input) =>
		Effect.succeed(
