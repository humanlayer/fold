 ...
 * the whole roster follows one `FoldSession.setProfile` swap with no roster rebuild.
 */
import {
	defineSubagent,
	skillTool,
...
export const BASH_SUBAGENT_PROMPT: string =
	...
	'- Prefer `rg` over `grep` for content search and `fd` over `find` for filename search (fold ' +
...
export const AST_GREP_OUTLINE_GUIDANCE: string =
	...
	'`ast-grep outline` needs ast-grep 0.44.0 or newer. fold installs `ast-grep` (along with `rg` and ' +
	'`fd`) into `~/.fold/bin` automatically, so it is usually already on your PATH. If ' +
