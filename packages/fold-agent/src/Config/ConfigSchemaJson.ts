 ...
 * when absent, never clobbering an existing config) - the `fold config init` primitive.
 */
...
export const starterConfigJsonc = (): string =>
	`{
	// fold configuration. Generated by \`fold config init\`; edit freely.
	...
	// Named profiles selected with \`fold --profile <name>\`; the top-level "roles" stays the default.
...
export type ConfigInitResult = {
	...
	/** Path of the OAuth credential store (created EMPTY when absent; filled by `fold auth codex login`). */
 ...
 * and behind `fold config init`: create the directory, (re)write the version-matched
 ...
 * `fold auth codex login` runs or the `apiKeyEnv` variables are exported. Only the two generated
...
/** `fold config init`: the same bootstrap, kept under its command-facing name. */
