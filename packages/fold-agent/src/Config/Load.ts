/**
 * This file loads and decodes `~/.fold/config.jsonc` (D25). JSONC is JSON with `//` and block comments
 * and trailing commas; {@link stripJsonc} removes both (string-aware, so `//` or a comma inside a JSON
 * string is preserved) and the result is parsed as JSON, then decoded through the `FoldConfig` schema
 ...
 * schema's formatted message). The filesystem seam matches the rest of fold-agent (`fileSystem`
...
export type LoadConfigOptions = {
	/** Explicit config file path. Defaults to `<foldHome>/config.jsonc`. */
...
export const defaultFoldHome = (): string => join(homedir(), '.fold')

/** The config file path for a home directory. */
export const defaultConfigPath = (foldHome?: string): string => join(foldHome ?? defaultFoldHome(), 'config.jsonc')

/** Resolve the config path from loader options. */
export const configPathFor = (options?: LoadConfigOptions): string =>
	options?.path ?? defaultConfigPath(options?.foldHome)
...
export const loadFoldConfig = (
	...
	Effect.gen(function* () {
		...
		return yield* parseFoldConfig(text, { path })
	})

/**
 * Like {@link loadFoldConfig} but returns `null` when the file is absent (parse/decode errors still
