/**
 * This file defines the `FoldConfig` Effect Schema (D25): the single `~/.fold/config.jsonc` shape,
...
/**
 * One provider connection profile. `codex` needs no credential here (it uses `~/.fold/auth.json`);
...
export const ProviderConnection = Schema.Struct({
	...
	/** Model ids explicitly exposed by this connection (for custom/private provider catalogs). */
	configuredModels: Schema.optionalKey(
		Schema.Array(Schema.String).annotate({
			description: 'Model ids explicitly configured for this provider, in addition to catalog models',
		}),
	),
...
/**
 * Mode names a profile may pin. A config-side literal copy of `FOLD_MODE_NAMES` (Mode/ModeName.ts) -
...
/** Auto-compaction policy for sessions launched through fold-agent. */
...
/** Runtime stop conditions for sessions launched through fold-agent. */
...
const FoldConfigFields = {
	 ...
	 * an agent mode. fold ships `ultraclaude` and `ultracodex` RLM presets in the starter config.
	 */
	...
	/** Auto-compaction policy. Omit to use fold-agent defaults (enabled today). */
	compaction: Schema.optionalKey(AutoCompactConfig),
	/** Runtime stop conditions. Omit to use fold-agent defaults. */
...
/** The whole `~/.fold/config.jsonc` document. Unknown top-level keys are rejected at decode (D25). */
export const FoldConfig = Schema.Struct(FoldConfigFields)
	.check(providersReferencedByRolesExist)
	.annotate({ identifier: 'FoldConfig', description: 'fold configuration (~/.fold/config.jsonc)' })
export type FoldConfig = typeof FoldConfig.Type
