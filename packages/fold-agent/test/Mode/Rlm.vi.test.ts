...
const namedModel = (modelId: string): FoldModel => {
...
it('maps mode names to mode values', () => {
	expect(FOLD_MODE_NAMES).toEqual(['default', 'rlm'])
...
it('formats RLM root archive instructions around bash delegation', () => {
	...
	expect(text).toContain('/tmp/fold/sess_1.jsonl')
