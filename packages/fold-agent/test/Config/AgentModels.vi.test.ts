/**
 * AgentModels role-resolution tests (D25): a config role resolves to a runnable FoldModel descriptor
...
it.effect('orchestrator falls back to smart when unbound', () =>
	Effect.gen(function* () {
		...
		const config = yield* parseFoldConfig(text)
...
it.effect('a reasoning level outside the catalog effort list fails, naming the supported levels', () =>
	Effect.gen(function* () {
		const config = yield* parseFoldConfig(configWithReasoning('minimal', 'off'))
...
it.effect('a reasoning level on a model whose entry says reasoning: false fails', () =>
	Effect.gen(function* () {
		const config = yield* parseFoldConfig(configWithReasoning('off', 'high'))
...
it.effect("level 'off' always resolves, even on a reasoning-incapable catalog model", () =>
	Effect.gen(function* () {
		const config = yield* parseFoldConfig(configWithReasoning('off', 'off'))
		const models = agentModelsFromConfig(config, {
			env: env({ ANTHROPIC_API_KEY: 'sk-live' }),
			catalog: validationCatalog,
		})
...
it.effect('a catalog-listed effort level resolves cleanly', () =>
	Effect.gen(function* () {
		const config = yield* parseFoldConfig(configWithReasoning('max', 'off'))
		const models = agentModelsFromConfig(config, {
			env: env({ ANTHROPIC_API_KEY: 'sk-live' }),
			catalog: validationCatalog,
		})
...
it.effect("a codex binding for gpt-5.6-sol with reasoning 'max' resolves against the baked catalog", () =>
	Effect.gen(function* () {
		const config = yield* parseFoldConfig(gpt56ConfigText('gpt-5.6-sol'))
