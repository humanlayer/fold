/**
 * Live integration against the real ChatGPT Codex backend using the credentials in ~/.tart/auth.json.
...
const authPath = join(homedir(), '.tart', 'auth.json')
...
const modelId = process.env.TART_CODEX_LIVE_MODEL ?? 'gpt-5.5'

describe.skipIf(skip)('codex live (skipped in CI or without a codex entry in ~/.tart/auth.json)', () => {
	it.live(
